...
结论：不会每次重建/更新整个三维场景体素体，而是基于“可见视锥 + 近表面带”的局部增量更新，并以固定频率执行。
...
`Assets/Anaglyph/LaserTag/ColocationManager.cs` (行 42-44)
```csharp
...
`Assets/Anaglyph/XRTemplate/Depth/EnvIgnorePlayer.cs` (行 8-16)
```csharp
